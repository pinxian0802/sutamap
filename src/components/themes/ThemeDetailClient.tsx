...
export function ThemeDetailClient({ theme, locations, checkedCount, friends, isLoggedIn, userCheckinPhotos }: Props) {
    ...
    <div className="max-w-md lg:max-w-none mx-auto lg:h-[100dvh] lg:flex lg:overflow-hidden">
      ...
      <div ref={mapWrapRef} className="overflow-hidden m-4 lg:ml-0 rounded-[16px] border border-[var(--line)] shadow-[0_10px_24px_-16px_rgba(45,74,107,0.4)] lg:order-2 lg:flex-1">
        <div className="relative w-full h-[320px] lg:h-full">
          ...
          />
          {friends.length > 0 && (
            <div className="absolute bottom-3 left-3 z-[500] flex items-center gap-[6px] px-[10px] py-[7px] rounded-[10px]"
              style={{ background: 'rgba(251,248,241,0.92)', backdropFilter: 'blur(8px)', border: '1px solid var(--line)', boxShadow: '0 4px 12px -4px rgba(45,74,107,0.25)' }}
            >
              <div className="flex -space-x-1.5">
                {friends.slice(0, 5).map(f => (
                  <span key={f.userId} className="w-5 h-5 rounded-full bg-ink2 text-white text-[9px] font-bold grid place-items-center border border-paper">
                    {f.username[0]?.toUpperCase()}
                  </span>
                ))}
              </div>
              <span className="text-[11px] text-sub">{dict.themes.friendsWorking}</span>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="lg:order-1 lg:w-[30%] lg:flex lg:flex-col lg:min-h-0 lg:pr-4">
        {/* header card: back + title + stats */}
        <div className="px-4 pt-0 pb-0 lg:pt-4 lg:pb-4 lg:pr-0 lg:flex-shrink-0">
        ...
        {/* scrollable content */}
        <div className="px-4 pt-0 pb-4 lg:pr-2 lg:pb-4 lg:flex-1 lg:min-h-0 lg:overflow-y-auto sm-scroll space-y-4">
